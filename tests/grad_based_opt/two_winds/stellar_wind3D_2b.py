...
mass_loss_rate = 1e-11 / 1.309 #2.965e-3 / 1e4 / T_orb * mass_source     #2.965e-3 / 1e6 / T_orb * mass_source
mass_loss_rate2 = 10 * mass_loss_rate
...
#### for full description (~10Myr)
# length_temp = 30  # 1 parsec in au
# mass_temp = 1
# velocity_temp = 10
# time_temp = length_temp / velocity_temp  #make time_temp as high as possible to have small code time units

# code_length = length_temp * u.parsec   ## LONG RUN
...
wind_params = WindParams(
    ...
    wind_injection_positions = jnp.array([[-R_forced/2, 0.0, 0.0],[R_forced/2, 0.0, 0.0]]),#jnp.array([[-0.02,0.0,0.0],[0.02,0.0,0.0]])
    # real_params = jnp.array([t_val, mass_rates_value, vel_scales_value])
)
cooling_params = CoolingParams(
    cooling_curve_params = SimplePowerLawParams(reference_temperature = 1e16)
)
...
print("Physical mass loss rate2: ", (mass_loss_rate2 * code_mass / (code_units.code_time)).to(u.M_sun / u.yr))
...
if multi_gpu:
    ...
    result = time_integration(primitive_state, config, params, helper_data, registered_variables, sharding = named_sharding)
else:  
...
save_path = os.path.join('diff_winds', str(num_cells))
import numpy as np
...
ax1.plot((helper_data.r[num_cells // 2, :, num_cells // 2].flatten()*(code_units.code_length)).to(u.au).value, (final_state[registered_variables.density_index, num_cells // 2, :, num_cells // 2].flatten()*(code_units.code_density)).to(u.g/u.cm**3).value, label="Final Density") #, s = 1)
# ax1.set_xscale("log")
ax1.set_yscale("log")
ax1.set_xlabel("x [au]")
...
v_r = jnp.sqrt(final_state[registered_variables.velocity_index.x, num_cells // 2, :, num_cells // 2] ** 2 + final_state[registered_variables.velocity_index.y, num_cells // 2, :, num_cells // 2] ** 2 + final_state[registered_variables.velocity_index.z, num_cells // 2, :, num_cells // 2] ** 2)

ax2.plot((helper_data.r[num_cells // 2, :, num_cells // 2].flatten()*(code_units.code_length)).to(u.au).value, (v_r.flatten()*(code_units.code_velocity)).to(u.km/u.s).value, label="Radial Velocity")#, s = 1)
ax2.set_xlabel("x [au]")
ax2.set_ylabel("Velocity [km/s]")

# plot P / rho^gamma

ax3.plot((helper_data.r[num_cells // 2, :, num_cells // 2].flatten()*(code_units.code_length)).to(u.au).value, (final_state[registered_variables.pressure_index, num_cells // 2, :, num_cells // 2].flatten()*(code_units.code_pressure)).to(u.Pa).value, label="P / rho^gamma")#, s = 1)
ax3.set_xlabel("x [au]")
...
ani = animation.FuncAnimation(fig, animate, frames=len(result.states), interval=300)
...
ani = animation.FuncAnimation(fig, animate, frames=len(result.states), interval=intval)
plt.colorbar(ax.imshow(result.states[0][0, num_cells // 2, :, :].T, cmap="jet", origin="lower", extent=[0, box_size, 0, box_size], norm=LogNorm()), ax=ax)
...
def animate(i):
    ax.clear()
    state = result.states[i]
    im = ax.scatter(helper_data.r[num_cells // 2, :, num_cells // 2].flatten(), state[0, num_cells // 2, :, num_cells // 2].flatten(), s = 1)
    ax.set_yscale("log")
    ax.set_xlabel("x")
...
ani = animation.FuncAnimation(fig, animate, frames=n_frames, interval=intval)
