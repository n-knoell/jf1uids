...
ref_final_state = sample_simulation(
    reference_params.wind_mass_loss_rates[0],
    reference_params.wind_mass_loss_rates[1],
)
np.savez_compressed('ref_final.npz', arr=ref_final_state)
...
# generate a loss map
mass_loss_rates1 = jnp.logspace(
    -13,  
    -9,    
    60
)

mass_loss_rates2 = jnp.logspace(
    -12,
    -8,
    60
)
...
def gradient_descent_optimization(func, x_init, learning_rate=1e-12, tol=1e-12, max_iter=1000, min_iter = 40):
...
initial_guess1 = jnp.array([1e-10 / 1.309 * 1.8, 1e-12 / 1.309 * 10 * 1.3])
...
initial_guess2 = jnp.array([1e-11 / 1.309 * 0.3, 1e-12 / 1.309 * 10 * 1.8])
...
initial_guess3 = jnp.array([1e-10 / 1.309, 1e-10 / 1.309])    ###true: 1e-11 / 1.309, 1e-11 / 1.309 * 10
...
init3_final_state = sample_simulation(
    initial_guess3[0],
    initial_guess3[1],
)
np.savez_compressed('init3_final_state.npz', arr=init3_final_state)
print("SAVED init 3")
