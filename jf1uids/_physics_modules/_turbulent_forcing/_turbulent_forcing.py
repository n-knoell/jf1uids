...
def _apply_forcing(
    ...
    # forcing_info = forcing_vrms_and_tcross(
    #     primitive_state,
    #     amp,
    ...
    # )
    # jax.debug.print("t_cross_forcing_vol: {tcm}", tcm=forcing_info["t_cross_forcing_vol"])
    # jax.debug.print("t_cross_fluid_vol: {tcm}", tcm=forcing_info["t_cross_fluid_vol"])
...
def forcing_vrms_and_tcross(
    ...
    # forcing-only RMS (before amp)
    w_sq = wx_real**2 + wy_real**2 + wz_real**2
    rms_w_vol = jnp.sqrt(jnp.mean(w_sq))

    # actual forcing RMS velocity (field that is added each step)
    v_rms_forcing_vol = jnp.abs(amp) * rms_w_vol

    # fluid (gas) vrms: volume- and mass-weighted
    u = primitive_state[registered_variables.velocity_index.x]
    v = primitive_state[registered_variables.velocity_index.y]
    w = primitive_state[registered_variables.velocity_index.z]
    vel_sq = u**2 + v**2 + w**2
    vrms_fluid_vol = jnp.sqrt(jnp.mean(vel_sq))

    # injection scale (paper: Linj ≈ L0/2)
    L_inj = config.box_size * 0.5

    # crossing times using forcing-only and fluid vrms
    t_cross_forcing_vol  = jnp.where(v_rms_forcing_vol > 0.0, L_inj / v_rms_forcing_vol, jnp.inf)
    t_cross_fluid_vol    = jnp.where(vrms_fluid_vol > 0.0, L_inj / vrms_fluid_vol, jnp.inf)
    jax.debug.print("t_cross_forcing_vol shape: {}", t_cross_forcing_vol.shape)
    jax.debug.print("t_cross_fluid_vol shape: {}", t_cross_fluid_vol.shape)

    return {
        "t_cross_forcing_vol": t_cross_forcing_vol,
        "t_cross_fluid_vol": t_cross_fluid_vol,
    }
